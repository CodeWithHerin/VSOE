    ...
    return (
        <main className="min-h-screen pt-32 pb-20 px-6 max-w-7xl mx-auto flex items-center justify-center">
            <div className="w-full max-w-md bg-white/5 p-12 border border-white/10 backdrop-blur-md">
                <h1 className="text-3xl font-serif text-vsoe-cream mb-8 text-center">{t.forms.passengerLogin}</h1>

                <form action={async (formData: FormData) => {
                    await authenticate(formData);
                }} className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] uppercase tracking-widest text-vsoe-gold">{t.forms.email}</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] uppercase tracking-widest text-vsoe-gold">{t.forms.password}</label>
                    ...
                    <div className="pt-8">
                        <MagneticButton className="w-full bg-vsoe-gold text-vsoe-midnight py-4 text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-white transition-colors flex justify-center">
                            <button className="w-full h-full">{t.forms.signIn}</button>
                        </MagneticButton>
                    </div>
